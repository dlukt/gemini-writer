...
def compress_context_impl(
    ...
    2. Calls the GLM API to create a comprehensive summary
    3. Saves the summary to a timestamped file
    4. Returns the compressed messages list and stats

    Args:
        messages: The full message history (list of dicts with 'role' and 'content')
...
"""
    ...
    for msg in messages_to_compress:
        ...
        if role == "model" or role == "assistant":
            ...
            if msg.get("tool_calls") or msg.get("function_calls"):
                func_calls = msg.get("tool_calls") or msg.get("function_calls")
                tool_calls_info = []
    ...
    # Call the GLM API to get summary
    try:
